...
def train1(model, save_dir):
    ...
    y_train = get_multilabel_ZangFu(y_train)
    y_test = get_multilabel_ZangFu(y_test)
...
def train2(model, save_dir):
    ...
    # y_test = np_utils.to_categorical(y_test,)
    #get 多标签分类
    y_train = get_multilabel_BaGang(y_train)
    y_test = get_multilabel_BaGang(y_test)
...
def train3(model, save_dir):
    ...
    # y_test = np_utils.to_categorical(y_test,)
    #get 多标签分类
    y_train = get_multilabel_QiXueJinYe(y_train)
    y_test = get_multilabel_QiXueJinYe(y_test)
...
def train4(model, save_dir):
        ...
        # y_test = np_utils.to_categorical(y_test,)
        # get 多标签分类
        y_train = get_multilabel_WeiQiYingXue(y_train)
        y_test = get_multilabel_WeiQiYingXue(y_test)
...
def train5(model, save_dir):
    ...
    # y_test = np_utils.to_categorical(y_test,)
    #get 多标签分类，这里是不同的train函数的主要区别
    y_train = get_multilabel_SanJiao(y_train)
    y_test = get_multilabel_SanJiao(y_test)
...
def test2(model, save_dir):
    ...
    print(type(y_label), np.shape(y_label))
    y_str = y_label.astype(int)
    y_str = get_inverse_multilabel_BaGang(y_str)
    print(y_str)
    accuarcy = accuarcy_xsh(y_label,y_test,model.config.num_classes)
    print('accuarcy', accuarcy)
    score1 = f1_score(y_label,y_test,average='weighted')
...
def test3(model, save_dir):
    ...
    print(type(y_label), np.shape(y_label))
    y_str = y_label.astype(int)
    y_str = get_inverse_multilabel_QiXueJinye(y_str)
    print(y_str)
    accuarcy = accuarcy_xsh(y_label,y_test,model.config.num_classes)
    print('accuarcy', accuarcy)
    score1 = f1_score(y_label,y_test,average='weighted')
...
def test4(model, save_dir):
    ...
    print(type(y_label), np.shape(y_label))
    y_str = y_label.astype(int)
    y_str = get_inverse_multilabel_WeiQiYingXue(y_str)
    print(y_str)
    accuarcy = accuarcy_xsh(y_label,y_test,model.config.num_classes)
    print('accuarcy', accuarcy)
    score1 = f1_score(y_label,y_test,average='weighted')
